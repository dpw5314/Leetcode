...
class Solution {
public:
    void calculate (vector<vector<int>>& ans, TreeNode* root, int target, int sum, vector<int>& tempvec){
        sum += root->val;
        tempvec.push_back(root->val);
        if(!root->left and !root->right and sum == target) ans.push_back(tempvec);
        if(root->left) calculate(ans, root->left, target, sum, tempvec);
        if(root->right) calculate(ans, root->right, target, sum,  tempvec);
        
        tempvec.erase(tempvec.end() - 1); // or tempvec.pop_back();
        return;
    }
    vector<vector<int>> pathSum(TreeNode* root, int sum) {
        ...
        vector<int> tempvec;
        if(!root) return ans;
        calculate(ans, root, sum, 0, tempvec);
        return ans;
    }
};
