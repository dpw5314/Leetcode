...
*/
    ...
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        int size1 = postorder.size(), size2 = inorder.size();
        TreeNode* ans;
        reverse(postorder.begin(), postorder.end());
        if(size1 != size2) return nullptr;
        else if(size1 == 0) return nullptr;
        else if(size1 == 1){
            ans = new TreeNode(postorder[0]);
            return ans;
        }
        else{
            int index2 = 0;
            while(index2 < size2){
                if(inorder[index2] == postorder[0]) break;
                index2++;
            }
            ans = new TreeNode(postorder[0]);
            Build(ans, 0, index2, 0, size1, postorder, inorder);
        }
