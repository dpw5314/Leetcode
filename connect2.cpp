...
class Solution {
public:
    void run(TreeLinkNode* root, vector<vector<TreeLinkNode*>>& temp, int layer){
        int size = temp.size();
        root->next = NULL;
        if(size == layer){         
            vector<TreeLinkNode*> tempTree;
            tempTree.push_back(root);
            temp.push_back(tempTree);
        }
        else{
            temp[layer][temp[layer].size()-1]->next = root;
            temp[layer].push_back(root);
        }
        if(root->left) run(root->left, temp, layer+1);
        if(root->right) run(root->right, temp, layer+1);
        return;
    }
    void connect(TreeLinkNode *root) {
        TreeLinkNode* dummy = root;
        vector<vector<TreeLinkNode*>> temp;
        if(root) {
            run(dummy, temp, 0);
            dummy->next = NULL;
        }
