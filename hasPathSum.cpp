...
class Solution {
public:
    void calculate(bool& ans, TreeNode* root, int target, int sum){
        sum += root->val;
        //cout<<sum<<endl;
        if(!root->left and !root->right and sum == target) ans = 1;
        if(root->left) calculate(ans, root->left, target, sum);
        if(root->right) calculate(ans, root->right, target, sum);
        return;
    }
    bool hasPathSum(TreeNode* root, int sum) {
        bool ans = 0;
        if(!root) return ans;
        calculate(ans, root, sum, 0);
        return ans;
    }
};
