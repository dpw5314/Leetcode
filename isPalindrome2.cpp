...
class Solution {
public:
	bool find(string s, int front, int end, int limit){
		int delTime = 0;
       while(end>front){     	
        	if(delTime > limit) return false;
        	if(s[front] == s[end]){
        		//cout<<s[end]<<" ";
        		end--;
        		front++;
        			...
        			if((s[end-1] == s[front]) and (s[end] == s[front+1])){
        				if(delTime == limit) return false;
        				//cout<<endl<<"layer 2:"<<endl;
        				return find(s, front, end-1, 0)+find(s, front+1, end, 0);
        			}
        			else if (s[end-1] == s[front]){
        				end--;
        				delTime++;
        			}
        			else if (s[end] == s[front+1]){
        				front++;
        				delTime++;
        			}
        			else return false;
        ...
        if(s[front] == s[end]) return true;		
	}

    bool validPalindrome(string s) {
        if(s.size() == 1) return true;
        int front = 0, end = s.size()-1;
 		return find(s, front, end,1);
    }
};
