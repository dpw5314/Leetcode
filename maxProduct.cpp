...
public:
    int maxProduct(vector<int>& nums) {
        ...
        for(int i = 1; i < nums.size(); ++i){
            temp[0] = nums[i];
            temp[1] = nums[i]*vec[i-1];
            temp[2] = nums[i]*min[i-1];
            vec[i] = *max_element(temp.begin(), temp.end());
            min[i] = *min_element(temp.begin(), temp.end());
        }
        PrintItvector(vec);
        PrintItvector(min);
...
int main(){
	Solution a;
	vector<int> nums = {0,1,3,4,-1,5,6,7,-6,-100,-1,-2,7,3,-6};
	//{-2, 3, -4};
	//{-1,-2,-9,-6};
