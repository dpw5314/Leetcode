...
public:
    int maxProfit(vector<int>& prices) {
        int ans = 0;
        if(prices.size() == 0 or prices.size() == 1) return ans;
        int low = prices[0], high = 0;
        int size = prices.size();
        for(int i = 0; i<size-1; i++){
            if(prices[i+1] < prices[i] and high <= low) low = high = prices[i+1];
            else if (prices[i+1] < prices[i] and high > low and i<size-1) {ans += high - low; high = low = prices[i+1];}
            else if (prices[i+1] > prices[i] and prices[i+1] > high) high = prices[i+1];
            
            if(i == size-2 and high > low)ans += high - low;
