...
class Solution {
public:
    void sum(TreeNode* root, int preNum, int& ans){
        preNum = preNum*10 + root->val;
        if(!root->left and !root->right) ans += preNum;
        else{
            if(root->left) sum(root->left, preNum, ans);
            if(root->right) sum(root->right, preNum, ans);     
        }                 
        return;
    }
    int sumNumbers(TreeNode* root) {
        int ans = 0;
        if(root) sum(root, 0 ,ans);
        return ans;
    }
};
